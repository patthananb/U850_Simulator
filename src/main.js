  ...
  <main>
    ...
    <aside class="process panel">
      ...
      <ol class="sequence">
        ...
        <li data-phase="transfer"><span class="step-number">02</span><div><strong>Load the flasher</strong><small>Transfer · seat · blow off</small></div><span class="phase-indicator"></span></li>
        <li data-phase="flash"><span class="step-number">03</span><div><strong>Program & verify</strong><small>Tool clear · vacuum off</small></div><span class="phase-indicator"></span></li>
        <li data-phase="retrieve"><span class="step-number">04</span><div><strong>Retrieve the IC</strong><small>Suction · lift from socket</small></div><span class="phase-indicator"></span></li>
        <li data-phase="sort"><span class="step-number">05</span><div><strong>Sort by result</strong><small>Good or fail · release · retract</small></div><span class="phase-indicator"></span></li>
      </ol>
...
function renderUI() {
  ...
  const phaseNames = ['pick', 'transfer', 'flash', 'retrieve', 'sort'], phaseIndex = phaseNames.indexOf(sim.phase?.group);
...
$('export').onclick = () => {
  const report = { schemaVersion: 1, simulationOnly: true, exportedAt: new Date().toISOString(), model: 'Concept visualization; not calibrated U850 kinematics', configuration: sim.config, status: sim.status, simulatedSeconds: sim.elapsed, results: { good: sim.good, fail: sim.fail }, parts: sim.parts, events: [...sim.logs].reverse() };
